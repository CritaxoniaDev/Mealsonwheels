...
<div class="bg-gray-100 py-12" id="update-member-information">
    ...
    <div class="container mx-auto px-4">
        <h2 class="font-semibold text-2xl text-gray-600 text-center">Update Member Information</h2>
        ...
        <div class="bg-white rounded shadow-lg p-4 px-4 md:p-8 mb-6">
            <div class="grid gap-4 gap-y-2 text-sm grid-cols-1 lg:grid-cols-3">
                ...
                <div class="lg:col-span-2">
                    <form action="{{ route('member#updateUserProfile', $userData->id) }}" method="POST">
                        @csrf
                        <div class="grid gap-4 gap-y-2 text-sm grid-cols-1 md:grid-cols-5">
                            ...
                            <div class="md:col-span-5">
                                ...
                                <input type="email" name="email" id="email" class="bg-gray-200 h-10 border mt-1 rounded px-4 w-full bg-gray-50" value="{{ old('email', $userData->email) }}" />
                            </div>
        ...
        <div class="bg-white rounded shadow-lg p-4 px-4 md:p-8 mb-6">
            <div class="grid gap-4 gap-y-2 text-sm grid-cols-1 lg:grid-cols-3">
                ...
                <div class="lg:col-span-2">
                    <form action="{{ route('member#updateCaregiverProfile', $userData->id) }}" method="POST">
                        @csrf
                        <div class="grid gap-4 gap-y-2 text-sm grid-cols-1 md:grid-cols-5">
                            <div class="md:col-span-5">
                                <label for="member_caregiver_name" class="font-bold uppercase">Care Giver's Name</label>
                                <input type="text" name="member_caregiver_name" id="member_caregiver_name" class="h-10 border mt-1 rounded px-4 w-full bg-gray-50" value="{{ old('member_caregiver_name', $memberData->member_caregiver_name) }}" />
                            </div>

                            <div class="md:col-span-5">
                                <label for="member_caregiver_relation" class="font-bold uppercase">Care Giver's Relationship</label>
                                <input type="text" name="member_caregiver_relation" id="member_caregiver_relation" class="h-10 border mt-1 rounded px-4 w-full bg-gray-50" value="{{ old('member_caregiver_relation', $memberData->member_caregiver_relation) }}" />
                            </div>

                            <div class="md:col-span-5">
                                <label for="member_medical_condition" class="font-bold uppercase">Medical Condition</label>
                                <input type="text" name="member_medical_condition" id="member_medical_condition" class="h-10 border mt-1 rounded px-4 w-full bg-gray-50" value="{{ old('member_medical_condition', $memberData->member_medical_condition) }}" />
                            </div>

                            <div class="md:col-span-3">
                                <label for="member_medical_number" class="font-bold uppercase">Medical Number</label>
                                <input type="text" name="member_medical_number" id="member_medical_number" class="h-10 border mt-1 rounded px-4 w-full bg-gray-50" value="{{ old('member_medical_number', $memberData->member_medical_number) }}" />
                            </div>

                            <div class="md:col-span-2">
                                <label for="member_meal_duration" class="font-bold uppercase">Duration (Read Only)</label>
                                <input type="number" name="member_meal_duration" id="member_meal_duration" class="h-10 border mt-1 rounded px-4 w-full bg-gray-50" value="{{ old('member_meal_duration', $memberData->member_meal_duration) }}" readonly />
                            </div>
